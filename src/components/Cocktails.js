...
class Cocktail extends Component {
    ...
    saveOffline = () => {
        ...
        // const pageResources = [`https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${this.state.cocktail.idDrink}`];

        // // Add images to the array
        // for (let i = 0; i < imageArray.length; i++) {
        //     pageResources.push(imageArray[i].src);
        // }
        // console.log(caches.keys().then((key)=> console.log(key)));

        // // Open the unique cache for this URL
        // caches.open('offline-' + this.state.cocktail.idDrink).then(function (cache) {
        //     const updateCache = cache.addAll(pageResources);
        //     // this.setState({ offlineData: cocktailData});

        //     updateCache.then(function () {
        //         console.log('available offline.');
        //     }).catch(function (error) {
        //         console.log('saved offline.', error);
        //     });
        // });
    ...
    render() {
        ...
        if(this.state.isOffline) {
            mainContent = <div>You seem offline! Go to <Link to="/saved">Saved Cocktails</Link></div>
        } else {
            mainContent = (
                <React.Fragment>
                    <CocktailInfo
                        ...
                        btnData={this.state.isSaved ? <FilledBookmarkIcon /> : <BookmarkIcon />}
                        isMainPage={true} />
